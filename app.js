document.addEventListener("DOMContentLoaded", () => {
    ...
    // 12. MAILTO FORM HANDLERS (Reporteros & Obituarios)
    const formReporteros = document.querySelector('form[name="noticias-ciudadanas"]');
    const formObituarios = document.querySelector('form[name="obituario-mascotas"]');

    if (formReporteros) {
        formReporteros.addEventListener("submit", (e) => {
            e.preventDefault();
            const nombre = document.getElementById("rep-nombre").value;
            const email = document.getElementById("rep-email").value;
            const telefono = document.getElementById("rep-telefono").value || "No especificado";
            ...
            const mailtoLink = `mailto:contacto@laespecie.cl?subject=${encodeURIComponent(`[NOTICIA DEL PÚBLICO] - ${titulo}`)}&body=${encodeURIComponent(
`Hola Equipo de La Especie,

Quiero reportar una noticia ciudadana:

- Nombre del Corresponsal: ${nombre}
...
*(Por favor, no olvides adjuntar tus fotos o videos a este correo antes de enviarlo)*

Saludos cordiales.`
            )}`;

            window.location.href = mailtoLink;
            if (modalReporteros) modalReporteros.style.display = "none";
            formReporteros.reset();
        });
    }

    if (formObituarios) {
        formObituarios.addEventListener("submit", (e) => {
            e.preventDefault();
            const mascota = document.getElementById("obi-mascota").value;
            const anos = document.getElementById("obi-anos").value;
            const familia = document.getElementById("obi-familia").value;
            const email = document.getElementById("obi-email").value;
            const epitafio = document.getElementById("obi-epitafio").value;

            const mailtoLink = `mailto:contacto@laespecie.cl?subject=${encodeURIComponent(`[OBITUARIO] - En Memoria de ${mascota}`)}&body=${encodeURIComponent(
`Hola Equipo de La Especie,

Quiero publicar un obituario en El Cielo de las Mascotas:

- Nombre de la Mascota: ${mascota}
- Años de Vida (Nacimiento - Partida): ${anos}
- Familia que lo despide: ${familia}
- Correo de contacto: ${email}

Epitafio / Mensaje de Despedida:
"${epitafio}"

*(Por favor, no olvides adjuntar la fotografía de tu mascota a este correo antes de enviarlo)*

Saludos cordiales.`
            )}`;

            window.location.href = mailtoLink;
            if (modalObituario) modalObituario.style.display = "none";
            formObituarios.reset();
        });
    }
});
